const interactiveScript = () => {
  ...
  const elements = {
    ...
    presets: Array.from(root.querySelectorAll('.p02-preset')),
  ...
  const selectPresetButton = preset => {
    (elements.presets || []).forEach(btn => {
      const isActive = btn === preset;
      btn.classList.toggle('toggle-active', isActive);
      btn.classList.toggle('toggle-inactive', !isActive);
      btn.setAttribute('aria-pressed', isActive ? 'true' : 'false');
    });
  ...
  const applyPreset = preset => {
    if (!preset) return;
    if (elements.accuracy && preset.dataset.accuracy) {
      elements.accuracy.value = preset.dataset.accuracy;
    }
    if (elements.structure && preset.dataset.structure) {
      elements.structure.value = preset.dataset.structure;
    }
    if (preset.dataset.delta) {
      selectDeltaButton(preset.dataset.delta);
    }
    selectPresetButton(preset);
    updateBase();
    updateSummary();
  };
  ...
  const updateBase = () => {
    ...
    if (elements.baseNote) {
      if (lowerBound <= 0.05) {
        ...
        elements.baseNote.textContent = 'Even strong base models keep a noticeable hallucination floor; the paper shows calibration alone cannot remove it.';
      } else {
        elements.baseNote.textContent = 'Large misclassification or calibration drift guarantees a high hallucination floor, matching the paper warning.';
      }
    }
  };
  ...
  const updateSingleton = () => {
    ...
    if (elements.singletonNote) {
      if (singletonPercent === 0) {
        elements.singletonNote.textContent = 'If every fact appears frequently, the singleton bound vanishes.';
      } else if (bound <= 0.08) {
        elements.singletonNote.textContent = 'Good coverage keeps the inevitable misses under 10%.';
      } else {
        elements.singletonNote.textContent = 'Lots of once-seen facts force a large error floor; the paper recommends gathering more evidence rather than trusting the model.';
      }
    }
  };
  ...
  const scoringRules = {
    binary: {
      reward: 1,
      penalty: 0,
      blurb: confidence => `Binary grading rewards guessing: expected score is higher when you answer even at ${Math.round(confidence * 100)}% confidence.`
    },
    'minus-one': {
      reward: 1,
      penalty: 1,
    ...
    confidence: {
      ...
      blurb: confidence => confidence >= 0.8
        ...
        : 'Confidence-target scoring penalises this guess, so abstaining earns more.'
    }
  };
  ...
  const updateRule = () => {
    ...
    const rule = scoringRules[state.rule];
    ...
    if (elements.ruleNote) {
      ...
      elements.ruleNote.classList.remove('panel-warning', 'panel-success');
      elements.ruleNote.classList.add(state.shouldGuess ? 'panel-warning' : 'panel-success');
    }
  };
  ...
  const updateSummary = () => {
    ...
    const baseMsg = state.baseBound <= 0.05
      ? `With ${accuracyPercent}% accuracy and tight calibration, the generative lower bound shrinks to ${formatPercent(state.baseBound)}, but any drift pushes it back up.`
    ...
    const singletonMsg = state.singletonBound === 0
      ...
      : `With ${singletonPercent}% singleton facts (about ${samplesK}k samples), Good-Turing predicts at least ${formatPercent(state.singletonBound)} errors on rare facts.`;

    const evalMsg = state.rule === 'binary'
      ? `Binary leaderboards keep rewarding guesses, so a ${confidencePercent}% confident answer is still attempted.`
      : state.shouldGuess
        ? `This scoring rule still prefers guessing at ${confidencePercent}% confidence, so abstentions stay rare.`
        : 'This scoring rule penalises bluffing here, pushing the model to abstain and match the paper recommendation.';
  ...
  if (elements.presets.length) {
    applyPreset(elements.presets[0]);
  }
